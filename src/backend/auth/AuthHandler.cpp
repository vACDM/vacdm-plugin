...
const AuthHandler::AuthProcessState AuthHandler::startAuthProcess() {
    const std::string test = Server::instance().m_baseUrl;
    const Json::Value emptyJson = Json::Value(Json::objectValue);
    auto response = Server::instance().postMessage(Server::instance().m_baseUrl + "/api/plugin-token/start", emptyJson);

    const auto emptyJsonString = emptyJson.toStyledString();
    ...
    std::string responseFormatted = response.toStyledString();
...
void AuthHandler::waitOnAuthProcessCompletion() {
    ...
    while (true) {
        ...
        responseFormatted = response.toStyledString();
